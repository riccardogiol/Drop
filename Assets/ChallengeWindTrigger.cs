...
public class ChallengeWindTrigger : ChallengeScript
{
    int windTriggLimit = 0;
    int type = 0;
    int windTriggSinceStart = 0;
    ...
    public void IncreaseCounter(int amount = 1)
    {
        if (stopCounter)
            return;
        windTriggSinceStart += amount;
        challengeInfo.WriteText(windTriggSinceStart + "/" + windTriggLimit + " winTrigg");
        if (!recordChallengeWon)
        {
            if (currentState != 1 && windTriggSinceStart <= windTriggLimit)
            {
                currentState = 1;
                challengeInfo.SetMedalState(1);
            } else if (currentState != 0 && windTriggSinceStart > windTriggLimit)
            {
                currentState = 0;
    ...
    {
        stopCounter = stop;
        return new ChallengeResults(windTriggSinceStart <= windTriggLimit, windTriggLimit, windTriggSinceStart, "lessThan");
    }
